...
1. **テストシナリオを作成** - AI Agentに自然言語で指示、Baselineも同時に取得
2. **リポジトリにコミット** - シナリオとBaselineをチームで共有
3. **差分検証** - 以後の実行でBaselineとの差分を検出

## Step 1: テストシナリオの作成

AI Agentに自然言語でテストしたい内容を伝えるだけで、シナリオ作成からBaseline取得まで自動で行います。

> **注意**: テストケース作成時、AI Agentは**アプリの実装コードを変更しません**。テストは既存の動作を検証するためのものです。
