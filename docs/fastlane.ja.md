...
lane :visual_tests do
  ...
  # SnapDriveテストを実行
  sh("npx snapdrive-ios-cli run --all --snapdrive-dir ./.snapdrive")
end
...
lane :update_baselines do
  ...
  sh("npx snapdrive-ios-cli run --all --update-baselines --snapdrive-dir ./.snapdrive")
end
```
...
lane :parallel_visual_tests do
  ...
  test_cases.each do |test_case|
    sh("npx snapdrive-ios-cli run #{test_case} &")
  end
...
lane :multi_device_tests do
  ...
  devices.each do |device|
    ...
    sh("npx snapdrive-ios-cli run --all --device $(xcrun simctl list devices | grep '#{device}' | grep -oE '[A-F0-9-]{36}')")
  end
end
```
